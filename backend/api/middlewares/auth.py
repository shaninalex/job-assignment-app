...
from pkg.consts import AuthStatus, Role
    ...
    try:
        ...
        if "sub" not in claims:
            return response.error_response(
                None, ["Invalid claims"], status=HTTPStatus.UNAUTHORIZED
            )
        ...
        async with request.app["session"] as session:
            ...
            if not user:
                return response.error_response(
                    None, ["User not found"], status=HTTPStatus.UNAUTHORIZED
                )
        ...
        return response.error_response(
            None, ["Expired"], status=HTTPStatus.UNAUTHORIZED
        )

    except jwt.InvalidTokenError:
        return response.error_response(
            None, ["Invalid token"], status=HTTPStatus.UNAUTHORIZED
        )
...
def roles_required(roles: List[Role]):
    ...
    async def role_required_inner(request, handler):
        if request["user"].role not in roles:
            return response.error_response(
                None, ["Invalid token"], status=HTTPStatus.FORBIDDEN
            )
        return await handler(request)
